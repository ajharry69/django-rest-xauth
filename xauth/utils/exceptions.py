...
def wrap_error_response_data(ed, **kwargs):
    ...
    if is_valid_str(ed) and delimiter in ed:
    ...
    if is_valid_str(msg) and is_valid_str(detail) and msg.lower() == detail.lower():
...
def exception_handler(exception, context):
    ...
    response = Response(data={
        'detail': '#'.join([x for x in exception.args if is_valid_str(x)]),  # only join strings
    }, status=status.HTTP_400_BAD_REQUEST) if response is None else response
    error_data = response.data
    error_data = wrap_error_response_data(error_data.get('detail') if isinstance(error_data, dict) else None, errors=exception.args, **error_data)
